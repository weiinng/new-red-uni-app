<template>
	<view class="container">
		<zhouweiNavbar home :bgColor="bgColor" :fontColor="fontColor" :title="video_info.video_name">
			<view slot="right" class="search_box" @tap="openSearch"></view>
		</zhouweiNavbar>
		<video id="tui-video" :src="video_info.video_src" :danmu-list="danmuList" enable-danmu danmu-btn controls
		...
		<view class="tui-container">
			...
			<view  v-if="video_info.is_comment == 0">
...
<script>
	...
	import zhouweiNavbar from "@/components/common-nav-Bar/zhouwei-navbar.vue"
	...
	export default {
		...
		data() {
			return {
				...
				isCollection: false,
				cmtList: [{
					avatar: "../../../static/images/news/list_2.jpg",
					nickname: "匿名用户",
					fabulous: 1562,
					isFabulous: false,
		...
		methods: {
			...
			get_post_videoinfo(){
				request.request({
					url:"/app/post/common/video_particulars",
					data:{
					...
					if(res.data.status == 200){
						this.video_info = res.data.info_item
			...
			btnCmt: function() {
				if(this.video_info.is_comment == 0){
					uni.navigateTo({
...
.search_box {
	width: 35upx;
	height: 35upx;
	background-image: url('../../static/home_ic_search.png');
